      ...
      if (isDiscordId) {
        ...
        if (user) {
          fields.push(
            ...
            { name: '📊 Database Info', value: '\u200b', inline: false },
            { name: 'Verified', value: user.isVerified ? '`Yes`' : '`No`', inline: true },
            { name: 'Roblox Username', value: user.robloxUsername ? `\`${user.robloxUsername}\`` : '`Not linked`', inline: true },
            { name: 'Roblox ID', value: user.robloxId ? `\`${user.robloxId}\`` : '`Not linked`', inline: true },
            { name: 'Scripts Used', value: `\`${user.statistics.scriptsUsed}\``, inline: true },
            { name: 'Last Seen', value: `<t:${Math.floor(user.lastSeen.getTime() / 1000)}:R>`, inline: true },
          );
          ...
          if (user.isVerified && user.robloxId) {
            ...
            if (robloxUser) {
              fields.push(
                { name: '\u200b', value: '\u200b', inline: false },
                { name: '🎮 Roblox Profile', value: '\u200b', inline: false },
                { name: 'Display Name', value: `\`${robloxUser.displayName}\``, inline: true },
                { name: 'Username', value: `\`${robloxUser.name}\``, inline: true },
                { name: 'Roblox Banned', value: robloxUser.isBanned ? '`Yes`' : '`No`', inline: true },
                { name: 'Account Created', value: `<t:${Math.floor(new Date(robloxUser.created).getTime() / 1000)}:R>`, inline: true },
          ...
          fields.push(
            { name: '\u200b', value: '\u200b', inline: false },
            { name: '📊 Database Info', value: '`Not registered in the database`', inline: false },
          );
        }
