...
import { createPollEmbed, createGiveawayEmbed, createSuccessEmbed, } from './embeds.js';
...
// ─── Public API ───────────────────────────────────────────────
export async function rehydrateScheduler(client) {
...
// ─── Polls ────────────────────────────────────────────────────
async function rehydratePolls(client) {
...
export async function schedulePollEnd(client, pollId, endTime) {
...
// ─── Giveaways ────────────────────────────────────────────────
async function rehydrateGiveaways(client) {
...
export async function scheduleGiveawayEnd(client, giveawayId, endTime) {
    const delay = endTime.getTime() - Date.now();
    if (delay < MIN_MS) {
...
async function endGiveaway(client, giveawayId) {
    try {
        ...
        if (giveaway.participants.length > 0) {
            winnerId = giveaway.participants[Math.floor(Math.random() * giveaway.participants.length)];
        }
