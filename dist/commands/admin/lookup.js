...
// ─── Discord Lookup ───────────────────────────────────────────
async function discordLookup(interaction, userId) {
    ...
    // ── Database section ──
    if (dbUser) {
        fields.push({ name: '\u200b', value: '**── Database ──**', inline: false }, { name: '✅  Verified', value: dbUser.isVerified ? '`Yes`' : '`No`', inline: true }, { name: '🎮  Roblox', value: dbUser.robloxUsername ? `[\`${dbUser.robloxUsername}\`](https://www.roblox.com/users/${dbUser.robloxId}/profile)` : '`Not linked`', inline: true }, { name: '📜  Scripts Used', value: `\`${dbUser.statistics.scriptsUsed}\``, inline: true }, { name: '👁️  Last Seen', value: ts.relative(dbUser.lastSeen), inline: true });
        ...
        if (dbUser.robloxId) {
            ...
            if (roblox) {
                fields.push({ name: '\u200b', value: '**── Roblox ──**', inline: false }, { name: '👤  Display Name', value: `\`${roblox.displayName}\``, inline: true }, { name: '📅  Account Created', value: ts.relative(new Date(roblox.created)), inline: true }, { name: '🔢  Account Age', value: RobloxAPI.accountAge(roblox.created), inline: true }, { name: '🚫  Banned', value: roblox.isBanned ? '`Yes` ⚠️' : '`No`', inline: true });
...
// ─── Roblox Lookup ────────────────────────────────────────────
async function robloxLookup(interaction, username) {
