...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('poll_id').setDescription('The poll ID').setRequired(true)))
        .addSubcommand(sub => sub.setName('list').setDescription('List all active polls')),
    async execute(interaction) {
        const sub = interaction.options.getSubcommand();
        if (sub === 'create')
            return createPoll(interaction);
        if (sub === 'end')
            return endPoll(interaction);
        if (sub === 'list')
            return listPolls(interaction);
    },
};
// ─── Create ───────────────────────────────────────────────────
async function createPoll(interaction) {
    ...
    try {
        ...
        const row = new ActionRowBuilder().addComponents(optionTexts.map((text, i) => new ButtonBuilder()
        ...
        if (poll.messageId && poll.channelId) {
            try {
                ...
                const msg = ch && 'messages' in ch
                    ? await ch.messages.fetch(poll.messageId)
                    : null;
                if (msg)
